/*
    ...
    Author: Maxim
    Version: 3.3.0 (Active Controller Standard)
...
const PE_V3 = (() => {
        ...
        injectIconButtons() {
            ...
            config.icon_buttons.forEach(btn => {
                ...
                if (btn.url.startsWith('#')) {
                    ...
                    // Add smooth scroll listener for internal links
                    el.addEventListener('click', (e) => {
                        e.preventDefault();
                        const targetId = btn.url;
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    const result = await core.API.post(
                        ...
                        button,
                        { demoMode: isDemo, demoKey: 'ex_search_not_found_demo' }
                    );
            ...
            form.addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    const result = await core.API.post(
                        ...
                        payload, 
                        button,
                        { demoMode: isDemo, demoKey: 'ex_contact_success_demo' }
                    );
    ...
    // [Active Controller Initialization]
    const initApp = async (siteConfig) => {
        ...
        // 1. Core Handshake (Strict Order)
        core = await Express.ready(siteConfig);
        config = core.config;
        currentLangData = core.Data.get();
        ...
        if (langSwitcher) {
            langSwitcher.addEventListener('click', async (e) => {
                ...
                if (button) {
                    ...
                    if (history.pushState) history.pushState({lang: selectedLang}, '', currentUrl.toString());
                }
            });
        }

        // 3. Canvas Injection via Core
        ...
        uiManager.init();
        forms.init();
        core.UI.initDemoLinks(); // Re-bind for dynamic content if needed
        ...
        registerEffect: (name, effect) => {
            if(core && core.Effects) {
                core.Effects[name] = { init: effect };
                 ...
                 // Pre-init registration
                 if (!window.Express) window.Express = {};
                 if (!window.Express.Effects) window.Express.Effects = {};
                 window.Express.Effects[name] = { init: effect };
            }
        }
    };
})();