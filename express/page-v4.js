...
const PE_V4 = (() => {
    ...
    // -------------------------------------------------------------------------
    // 1. UI Manager (Navigation & Buttons)
    // -------------------------------------------------------------------------
    const UIManager = {
        init() {
            this.setupNav();
        ...
        setupNav() {
            ...
            const menuBtn = document.getElementById('mobile-menu-btn'); // If exists in V4 HTML
            ...
            if (langSwitcher) {
                langSwitcher.addEventListener('click', async (e) => {
                    ...
                    if (btn) {
                        ...
                        // Update DOM Text (Core Utility could handle this, but explicit here for V4)
                        this.renderLanguage();
                    }
                });
            }
        ...
        injectIconButtons() {
            // Target: .pe-header__content (V4 Standard)
    ...
    // -------------------------------------------------------------------------
    // 3. Hydration (Forms & Dynamic Content)
    // -------------------------------------------------------------------------
    const Hydration = {
        init() {
            this.hydrateForms();
        ...
        hydrateForms() {
            // [V4 Standard] Handle .pe-form (Contact / Search)
            core.Util.$$('.pe-form').forEach(form => {
                ...
                form.addEventListener('submit', async (e) => {
                    ...
                    let endpoint = 'conn.contact'; // Default
                    ...
                    try {
                        ...
                        if (statusEl) {
                            ...
                            statusEl.innerHTML = result.found ? 
        ...
        renderDynamicContent() {
            // Hydrate [data-content-type] sections (FAQ, Product List) using Core Data
            ...
            containers.forEach(container => {
                ...
                const prefix = container.dataset.prefix; // e.g., "faq_cake"
                let html = '';

                // Iterate up to 50 items
                for (let i = 1; i <= 50; i++) {
                    const idx = String(i).padStart(2, '0');
                    const labelKey = `${prefix}_${idx}`; // e.g., faq_cake_01
                    
                    // Check existence logic
                    let exists = false;
                    if (type === 'faq') exists = !!data[`${prefix}_${idx}_q`];
                    else exists = !!data[labelKey]; // product title

                    if (!exists) {
                        if (i > 1) break; // Stop if gap found after 01
                        continue;
                    }
                    ...
                    if (type === 'faq') {
                        ...
                        const mod = core.Util.getText(`${prefix}_${idx}_modifier`); // 'options'
                        const modClass = mod ? ` pe-product-list__item--${mod}` : '';
                        
                        html += `
                            <div class="pe-product-list__item${modClass}">
    ...
    const init = async (siteConfig) => {
        if (typeof Express === 'undefined') {
            console.error('Express Core (Tier 1) is missing.');
            return;
        }
        ...
        // Render Initial Language (Sync UI with Core Data)
        ...
        // Canvas Background Init (Tier 1 Canvas)
